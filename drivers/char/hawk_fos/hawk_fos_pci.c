...
static int fos_open(struct inode *i, struct file *filp)
{
   ...
   fos = container_of(i->i_cdev, struct fos_drvdata, cdev);
	...
	fos->auto_mig_addr = 0;
	fos->auto_row_stride = 0;
	fos->auto_channel_offset_bytes = 0;
   fos->auto_ch1_num_bytes = 0;
   fos->auto_ch2_num_bytes = 0;
   fos->auto_channel_mode = 0;
   fos->irq_count = 0;
...
//   u32      val;
   ...
   switch (cmd) {
      ...
      case FOS_AUTO_ACQUISITION:
         ret = FOS_Auto_Acquisition(fos, arg_ptr);
         return ret;

      case FOS_AUTO_BLOCK_COUNT:
         if (copy_to_user(arg_ptr, &fos->auto_block_count, sizeof(u32))) {
            return -EFAULT;
         }
         return ret;
...
irqreturn_t fos_isr(int irq, void *data)
{
   ...
   if (status & STAT_ARBITER_INT_FLAG){
		if(fos->auto_active == 1){
			// initiate transfer request for 1 channel
			if(fos->auto_channel_mode == CHANNEL2_MODE){
				tfer.num_columns = fos->auto_ch2_num_bytes;
				tfer.host_offset_addr = (fos->auto_ch2_num_bytes*fos->auto_mig_addr) + fos->auto_channel_offset_bytes;
			}else{
				tfer.num_columns = fos->auto_ch1_num_bytes;
				tfer.host_offset_addr = fos->auto_ch1_num_bytes*fos->auto_mig_addr;
			}
   ...
   if (status & STAT_MIG2HOST_INT_FLAG){
		//initiate 2nd channel transfer
		if(fos->auto_transfer_ch2 == 0){
			fos->auto_block_count = (fos->auto_block_count + 1)&7;
		}

		if(fos->auto_transfer_ch2 == 2){
			fos->auto_block_count = (fos->auto_block_count + 1)&7;
			fos->auto_transfer_ch2 = 0;
		}

		if((fos->auto_active == 1) && (fos->auto_transfer_ch2 == 1)){
			tfer.mig_base_address = (fos->auto_row_stride*fos->auto_mig_addr)+(fos->auto_row_stride/2);
			tfer.mig_stride = fos->auto_row_stride;
			tfer.num_columns = fos->auto_ch2_num_bytes;
			tfer.num_rows = 128;
			tfer.host_offset_addr = (fos->auto_ch2_num_bytes*fos->auto_mig_addr) + fos->auto_channel_offset_bytes;
...
static int fos_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
   ...
   fos_cma_area = dma_alloc_from_contiguous(&pdev->dev, ((DMA_LENGTH)>>PAGE_SHIFT), PAGE_SIZE, GFP_DMA | GFP_HIGHUSER);
   if (fos_cma_area) {
