...
int FOS_Auto_Acquisition(struct fos_drvdata *fos, void *user_ptr)
{
   struct FOS_auto_cmd_struct cmd;

   u32   max_num_bins_value,mode;
   u32   status;
#ifdef DEBUG
   printk(KERN_DEBUG "ioctl: entered FOS_Auto_Acquisition\n");
#endif
   if (copy_from_user(&cmd, user_ptr, sizeof(cmd))) {
      printk(KERN_DEBUG "FOS_Auto_Acquisition: copy failed\n");
   ...
   status = FOS_Status(fos);
#ifdef DEBUG
   printk(KERN_DEBUG "ioctl: FOS_Auto_Acquisition, status = 0x%x\n",status);
#endif
   if (status & SWEEP_RUNNING_FLAG)
      return -2;
   ...
   // write out COTDR format data
   max_num_bins_value = cmd.max_num_bins + 31;
   max_num_bins_value /= 32;
   max_num_bins_value *= 2;
   ...
   // end at address block 1023
   fos_write_reg(fos, R_BOTDA_END_FREQ, REPEAT_LOOP|INTERRUPT_ACTIVE|REPEAT_COUNT);
   fos_write_reg(fos, R_ADC_COUNT, max_num_bins_value);
   ...
   if (cmd.capture == START_CAPTURE) {
      fos_write_reg(fos, R_RUN_TEST, START_TEST);
		...
		fos->auto_row_stride = cmd.row_stride;
		fos->auto_channel_offset_bytes 	= cmd.ch1_num_bins*2048;
		fos->auto_ch1_num_bytes 			= cmd.ch1_num_bins*2;
		fos->auto_ch2_num_bytes 			= cmd.ch2_num_bins*2;
		// enable mig2host & arbiter interrupt
